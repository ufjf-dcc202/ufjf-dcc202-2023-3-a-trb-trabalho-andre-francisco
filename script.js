    ...
    cellElement.addEventListener("click", function () {
        ...
        if (!cellElement.querySelector('img')) {
            ...
            if (turn === 1) {
                ...
                // Adiciona o valor da célula ao array do jogador 2
                cells2.push(dice);
                // Incrementr o número de células preenchidas pelo jogador 2
                filled2++;
                // Descarta os dados do jogador 1 que tenham o mesmo valor
                discard(1, dice);
                // Calcula a pontuação do jogador 2
                score2 = calculateScore(cells2);
                // Atualiza a pontuação do jogador 2 no HTML
...
// Adiciona o clique dass células dos jogadores
for (let i = 1; i <= 3; i++) {
    for (let j = 1; j <= 6; j++) {
...
function discard(player, value) {
    // Verificr qual jogador está sendo afetado
    if (player === 1) {
        ...
        // Percorre as células do jogador 2
        for (let i = 1; i <= 3; i++) {
            for (let j = 4; j <= 6; j++) {
                // Pegar a célula atual
                let cell = document.getElementById("cell" + i + j);
                // Verifica se a célula tem o mesmo valor do dado
                if (cell.querySelector('img') && cell.querySelector('img').alt === `dice${value}`) {
                    // Limpar a célula
                    cell.innerHTML = "";
...
function calculateScore(cells) {
    ...
    let score = 0;
    // Verifica se o array de células está vazio
    if (cells.length === 0) {
        // Retornar zero
        return score;
    }
    // Ordena o array de células em ordem crescente
    cells.sort((a, b) => a - b);
    // Inicializa o valor e a quantidade do dado atual
    let value = cells[0];
    let count = 1;
    // Percorre o array de células a partir do segundo elemento
    for (let i = 1; i < cells.length; i++) {
        // Verifica se o elemento atual é igual ao anterior
        if (cells[i] === value) {
            ...
            // Atualiza o valor e a quantidade do dado atual
            value = cells[i];
            count = 1;
        }
    }
